...
md"""
Realizo corridas sobre vectores $\theta$, $\theta'$ $\in \mathbb{R}^{n}$ tales que suman k, $\theta_i \leq \theta'_i \space$ $\forall i \in \{1, 2, 3\} \space$ y $\space \theta_i \geq \theta'_i$ $\forall i \in \{4, \dots, n\}$, para encontrar un contraejemplo a $\textit{selection monotonicity}$.
"""
...
# ╔═╡ a6ae60f3-3d34-4fc4-a871-946df4065398
begin
	p = rand_vector_sum3(9, float(k2))
	pp = perturb_vector(p)
	println(round.(p; digits=10))
	println(round.(pp; digits=10))
	println(sum(pp))
end
