...
methods fix the divisor and adjust the rounding rule." \cita{pukkelsheim2017}
...
tradicional de redondeo al entero más cercano. Por su parte, el método D'Hont utiliza la función "piso",
...
\begin{definition}
    ...
    \[
    f: [0,\infty) \rightarrow \mathbb{N} \ \text{creciente y sobreyectiva.}
    \]
\end{definition}

El hecho de que una función de redondeo sea creciente implica que 
...
\[
n \in \llbracket t \rrbracket \iff s(n) \leq t \leq s(n+1) \quad \forall n \in \mathbb{N}, t \geq 0
\]
...
Tabla extraída de \cita{pukkelsheim2017}}
\end{table}
...
\vspace{2em}
La idea de esta familia de métodos es "dividir y redondear" (\cite{pukkelsheim2017}), dividiendo los votos por cierto divisor 
...
Como a veces se utilizará el vector $\vec{w}$ de proporciones dado por $\vec{w}_i = \frac{\vec{v_i}}{v_+}$ en lugar de 
$\vec{v}$, tendrá sentido considerar también un multiplicador $\mu$ en lugar de solo el divisor $D$.


\begin{proposition}
    Los métodos de divisor satisfacen anonimidad, balance, concordancia, decencia y exactitud.
    \begin{proof}
        La demostración no es ni muy relevante ni muy instructiva, por lo que queda relegada al apéndice.
    \end{proof}
\end{proposition}
...
\begin{proposition}
    ...
    \begin{proof}
        Teniendo en cuenta que ambas condiciones son complementarias, basta con probar una sola de las dos equivalencias.
        
        Supongamos que 
        $$\max\limits_{j \in [n]} \frac{v_j}{s(x_j + 1)} = \min\limits_{j \in [n]} \frac{v_j}{s(x_j)}$$
        Luego, existe un único divisor $D > 0$ tal que $\sum_{i=1}^{n}{\llbracket \frac{v_i}{D} \rrbracket} = k$ tiene solución.
        Además, como 
    \end{proof}
\end{proposition}
