...
Dado que no es trivial ver que este método resulta \textit{proporcional ex-ante}, y tampoco se encuentra demostrado en \cita{correa2024monotonerandomizedapportionment}, 
procederemos a demostrarlo. Para esto, 
...
\begin{proof}
    ...
    \begin{align}
        &= \frac{
            \displaystyle\sum_{\substack{A \in \binom{[n]}{k} \\ 1 \in A}}
            \left(\displaystyle\sum_{i\in A}(1-p_i)\right)
            \displaystyle\prod_{j\in A} p_j
            \displaystyle\prod_{j\notin A}(1-p_j)
        }{
            \displaystyle\sum_{\substack{B \in \binom{[n]}{k} \\ 1 \in B}}
            \underbrace{
                \left(\sum_{i\notin B}(p_i)\right)
            }_{\textcolor{azulAcero}{\displaystyle\sum_{i\in B}(1-p_i) = \displaystyle\sum_{i\notin B}(p_i)}}
            \displaystyle\prod_{j\in B} p_j
            \displaystyle\prod_{j\notin B}(1-p_j)
            +
            \underbrace{
    ...
    Usando esta igualdad, por \ref{eq:prob-1-en-S} tenemos
    ...
    \begin{align*}
        \mathbb{P}(1 \in S)
            ...
            &= \frac{\alpha}{(1 + \frac{1-p_1}{p_1}) \alpha} \ , \hspace{1em} \text{ con } 
            \alpha \vcentcolon= \sum_{\substack{B \in \binom{[n]}{k} \\ 1 \in B}}
            \left(\sum_{i\notin B}p_i\right)
            \prod_{j\in B} p_j
            \prod_{j\notin B}(1-p_j) \\
            &= p_1.
...
El otro resultado relevante en relación al algoritmo de Sampford queda plasmado
en la siguiente propiedad:

\begin{proposition}
    El algoritmo de Sampford satisface \textit{monotonía de selección}.

    La demostración requiere varios lemas, y es el objeto central de \cita{correa2024monotonerandomizedapportionment}.
\end{proposition}


% ------------------------------------------------------
\subsection{Algoritmo de Brewer}

Demostración por inducción de que Brewer es Ex-Ante proportional.