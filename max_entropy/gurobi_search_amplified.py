...
Q = [x * Q_prod_normalization * 100 for x in Q]
...
Q_ = [x * Q__prod_normalization * 100 for x in Q_]
...
q_ = model.addVars(N, lb=eps, name="q_")
...
aux_ = {}
for s in all_sets:
    aux[s] = model.addVar(lb=0.0, name=f"aux_{s}")
    aux_[s] = model.addVar(lb=0.0, name=f"aux_{s}_prime")
...
for s in all_sets:
    ...
    m1 = model.addVar(lb=0.0, name=f"m1_{s}")
    m1_ = model.addVar(lb=0.0, name=f"m1_{s}_prime")
    bilin_aux[s] = m1
    bilin_aux_[s] = m1_
...
model.addConstr(aux_[S] <= aux[S] - 2*eps, name="main_prod_constraint")
...
for i in S:
    ...
    model.addConstr(lhs >= rhs + eps, name=f"ineq_S_{i}")

# Constraints: p(q_, i) <= p(q, i) for i in S_c
for i in S_c:
    lhs = gp.quicksum(aux_[s] for s in all_sets if i in s)
    rhs = gp.quicksum(aux[s] for s in all_sets if i in s)
    model.addConstr(lhs <= rhs - eps, name=f"ineq_Sc_{i}")

# Normalization constraints: sum of all monomials == 1
model.addConstr(gp.quicksum(aux[s] for s in all_sets) == 1e6, name="norm_q")
model.addConstr(gp.quicksum(aux_[s] for s in all_sets) == 1e6, name="norm_q_prime")
...
print("\nSum of aux Start:", sum(aux[i].Start for i in all_sets))
print("\nSum of aux_ Start:", sum(aux_[i].Start for i in all_sets))
